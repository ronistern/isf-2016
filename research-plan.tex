...
The importance of adequate MAP mechanisms for group activities is recognized in formal models of teamwork and is becoming increasingly important with the greater prevalence of MAS in practical applications.  %household and industrial robots as well as other types of MAS. %~\cite{grant2005formalApproaches}
...
MAP has many variants and many formal models have been proposed to describe a broad range of MAP problems, including COM-MTDP~\cite{pynadath2002communicative}, Multi-Agent STRIPS (MA-STRIPS)~\cite{brafman2013complexity}, TAEMS~\cite{Horling-182,lesser2004evolution}, Decentralized POMDP (Dec-POMDP)~\cite{bernstein2002complexity}, and its variants~\cite{becker2004decentralized,roth2007exploiting,oliehoek2012influenceBased}. It is widely acknowledged across all MAP formalisms that {\bf multi-agent planning is inherently more difficult than single-agent planning}, because agents need to reason not only about their individual plans but also about potential {\em interactions} between them, i.e., finding ways in which agents can help each other (positive interactions) and making sure that an agent executing one plan does not prevent the execution of another (negative interactions).
...
Most baseline planners embody the assumption that the set of potential interactions that need to be considered during planning are specified in, or easily derived from, the problem description given to the planner. For example, in MA-STRIPS~\cite{brafman2013complexity} and in Nearly Decomposable MDP (NeDMDP)~\cite{kamar2013modeling} the agents' actions are partitioned into private/individual actions and public/joint actions, and potential interactions occur only when performing public/joint actions. Another example is special variants of Dec-POMDP in which inter-agent influences are limited to the reward function~\cite{becker2003transition} 
or predetermined situations~\cite{varakantham2009exploiting,melo2011decentralized}, 
or are represented in a dynamic Bayesian network~\cite{nair2005networked,witwicki2010influence,oliehoek2012influenceBased}. 


% (having the agents' transitions and observations independent of each other), or to predetermined situations~\cite{varakantham2009exploiting,melo2011decentralized}, or 
...
\caption{\small{The vacuum cleaning robot, floor scrubbing robot, and floor mopping robot are marked by a circle with the letter V, S, and M, respectively. Collisions should be avoided and it is easier to scrub or mop the floor after it is vacuumed. Thus, the robots need to coordinate in order to clean the house efficiently. A possible joint plan starts with robot V vacuuming the floor at time=0 (vacuumed tiles are marked by V), while robot S moves towards that tile, eventually scrubbing it at time=3 (marked by adding the letter S on the tile) while robot M waits near by to avoid a collision. }}
\label{fig:example}
\end{figure}

{\bf Explicitly reasoning about all potential inter-agent interactions when planning severely limits the applicability of current MAP algorithms}, because the number of potential interactions can be large and MAP complexity is, roughly, exponential in the number of interactions reasoned about~\cite{brafman2013complexity,witwicki2011towards}. For example, in the above smart home scenario, each movement of one robot may interact with the movements of the other two robots by causing a collision. Thus, every move action of an individual robot raises a potential interaction (for example, in MA-STRIPS the ``move'' action would be a public action). % and most baseline planners will considered its interaction with the actions of all other robots when planning. % TODO: Reconsider the above sentence
...
Our overarching goal in this proposal is to deepen the understanding of MAP problems and develop a general-purpose algorithmic framework that enables planning for large and diverse groups of agents. 
...
\subsection{Objective \#2: Methods for Balancing Plan Cost and Plan Reliability with Planning Time}

The quality of a plan, whether individual or joint,  corresponds to the cost (or reward) of executing it. For joint plans, different ways of handling potential interactions result in different execution costs and require different computational efforts. Moreover, deciding not to reason about some interactions (see Section~\ref{sec:resolving}) may result in a plan being easier to compute but impossible to execute. 

Building on our prior work on suboptimal search~\cite{barer2014suboptimal-socs,stern2014potential,stern2011probably,stern2012search}, and that of others~\cite{pohl1973avoidance,pearl1982studies,thayer2011bounded}, we plan to develop  {\bf meta-reasoning mechanisms able to compute the trade-offs of both plan cost and plan reliability with planning time}. This capability is especially important because MAP problems can rarely be solved optimally in reasonable time.  Being able to quantify suboptimality of plan cost and probability of plan success (reliability) is an essential ingredient in deciding when a plan is good enough and execution can commence.




\subsection{Objective \#3: Methods for Choosing which Potential Interactions to Ignore when Planning}
%Algorithms for Identifying and Reason About Potential Interactions}

A fundamental tenet of IOP is that {\bf the mere possibility that agents may interact does not mean they should reason about that potential interaction when planning}. Ignoring potential interactions can result in higher execution costs or even mission failure, but in some case it can substantially reduce computation without such risks. Thus, an important objective of this research is to develop intelligent methods for deciding which potential interactions should be ignored when planning and handled on-the-fly during plan execution. These methods will provide a principled way for effective interleaving of planning and execution, a sought-after objective in online multi-agent planning~\cite{roth2007exploiting,wu2011online,kwak2011teamwork}. 
...
%In the above objectives the planning, while the execution may be distributed (each agent trying to follow its plan independantly) the planning itself is done in a centralized manner. While this is reasonable in many settings thanks to current high-speed communication networks, there are also cases where the planning too needs to be distributed. We will explore how to do so in IOP, building on message passing algorithm such as the one proposed by Nissim et al.~\cite{nissim2014distributed} and other protocols.  
...
\subsection{Objective \#4: IOP Planners for a Range of MAP Problems and Models}
%\item {\bf Demonstrate IOP in solving MA-STRIPS problems, and other MAP models.} 

While some of the components IOP comprises of have been studied in the past, they are scattered over different MAP algorithms and frameworks. A major contribution in the development of IOP is that it provides a unifying framework for all these MAP algorithm components, enabling greater flexibility and facilitating in-depth study of the trade-offs due to different MAP algorithm design choices. Thus, an experimental study of IOP in which we demonstrate this framework over a range of MAP problems is crucial to enhancing the impact of the proposed research. To this end, IOP-based planners will be implemented and experimentally studied in several types of MAP problems, including MA-STRIPS~\cite{brafman2013complexity}, search-based planning (SBPL) for teams of robots~\cite{likhachev2010sbpl}, 
Dec-POMDP~\cite{bernstein2002complexity}, 
and planning tasks for data-mules{~\cite{shah2003data,sugihara2011path,crowcroft2016using}. 
	Achieving this objective is challenging, as we have prior experience only in MA-STRIPS and planning for data-mules. Therefore, we plan to collaborate with other teams, and in particular with Prof. Maxim Likhachev (the creator of SBPL, see attached letter of support) and Prof. Christopher Amato (an expert in Dec-POMDP, see requested funding to support this). 

%MA-STRIPS, a now-popular model for domain-independent MAP, having public competitions~\cite{vstolba2015competition} and standard benchmarks, allowing fair comparison with other planners. Developing scalable planners for MA-STRIPS will increase the applicability of MAP algorithm, due to the generality of this model. 
...
%IOP is intended to be applicable to a wide range of MAP problems, and thus we define the type of MAP settings we address in general terms, as follows. %
The proposed research addresses the following type MAP problems. 
A group of agents agree to collaborate in order to achieve a set of goals (note, we do not consider malicious, adversarial, or self-interested agents). The output of a MAP algorithm is a {\bf joint plan}, which is a composite plan comprising a plan for each agent that specifies the actions it should take. 
This joint plan is passed to the agents, which start to execute it. 
We focus on the planning phase (as oppose to the execution phase)
and assume that the team members are known during planning (As oppose to ad-hoc teamwork~\cite{stone2013teaching}). Nonetheless, we explicitly consider that planning and execution may be interleaved by allowing the developed MAP algorithm to return a joint plan in which agents' plans are not fully specified. We explore this option in Section~\ref{sec:resolving}. 
...
%Viewing MAP as a process of continuous refinement is not novel, and in fact most planning algorithms can be described as a process of continual refinement~\cite{kambhampati1997refinement,lesser2004evolution,de2005multi,georgeff1988communication}. 
Most planning algorithms can also be described as a process of continuous refinement~\cite{kambhampati1997refinement,lesser2004evolution,de2005multi,georgeff1988communication}. 
...
(1) refinement corresponds to introducing coordination between plans (resolving PIFs), 
(2) the PIFs are computed and not given upfront as input (as done, for example, when specifying public/private actions in MA-STRIPS~\cite{brafman2013complexity} or coordination locales in some Dec-POMDP variants~\cite{varakantham2009exploiting}),
(3) multiple ways to resolve PIFs can be explored, following the understanding that no single approach to joint plan refinement is always preferable,
(4) the option of ignoring PIFs when planning is considered and reasoned about in a principled manner, and (5) IOP provides the mechanism for trading off the overall joint plan cost and reliability with planning time. 
While some previous work considered some of these features to some extent, they were never before studied and integrated in a single unified algorithmic framework. Next, we outline how we propose to develop the different components of IOP and the research challenges raised in their development. 
...
%The first algorithmic component of IOP we will study is the initial generation of provisional plans. As a baseline, 
We will explore several approaches for generating provisional plans (line~\ref{alg:generateProvisionalPlan} in Alg.~\ref{alg:IOP}):
...
Still, fully coordinated planning is a useful approach when all agents must coordinate tightly for the mission to succeed. 
...
{\bf 2. Uncoordinated planning.} {\em Assign goals to agents using existing task allocation mechanism~\cite{hunsberger2000combinatorial,sander2002scalable}, and then use a single-agent planner to generate a plan for every agent that achieves its allocated goals. }
Current single-agent planners are very efficient, so generating provisional plans this way is very efficient. However, PIFs may occur between such provisional plans  requiring additional plan refinements. Thus, we expect this approach to be useful when agents can reach their goals with minimal coordination. 
...
An important limitation of using uncoordinated planning to generate provisional plans is that it is incomplete, in the sense that the provisional plans may not achieve some goals. This occurs for goals that cannot be achieved by any individual agent without assistance from other agents. Detecting when this occurs can be done in some cases but is in general a hard problem~\cite{zhang2016formal}. Generating provisional plans by {\em planning under the generalized free-space assumption} is a middle ground between uncoordinated and coordinated planning, intended to resolve the incompleteness of uncoordinated planning without resorting to fully coordinated planning. Fast single-agent planners are used to generate the provisional plans, but these planners are modified to have some knowledge about what  other agents can achieve and assume their complete cooperation. This enables generating provisional plans even for goals that cannot be achieved without collaboration. 
...
{\bf 4. Coordinated planning for subsets of agents.} {\em Assign goals to agents as done in uncoordinated planning, partition the agents into groups and run a coordinated planner on each group independently.} 
The challenge in developing this hybrid approach is to develop smart algorithms for grouping agents intelligently, as we expect this approach to be especially effective if the agents grouped together indeed require tight coordination while coordination between agents in different groups is rarely required.  We implemented such an approach in prior work on the development of the MA-CBS algorithm for solving MAPF problems~\cite{sharon2015conflict-based}. In MA-CBS, we detect conflicts (agents wanting to occupy the same location at the same time) between agents during planning, and agents that conflict frequently are grouped and replanned together using coordinated planning. 

% and preliminary work on MA-STRIPS showed promise. 

{\bf 5. Coordinated planning of a relaxed MAP problem.} {\em Solve a relaxed version of the MAP problem using fully coordinated planning and then let the individual agents generate their provisional plans based on the joint plan generated for the relaxed problem.} Examples of possible MAP relaxations include by ignoring actions that do not directly affect other agents or ignoring delete effects of actions~\cite{hoffmann2005ignoring,domshlak2015red,vstolba2014relaxation}. We applied this approach successfully in our Greedy Privacy Preserving Planner (GPPP) for MA-STRIPS problems with privacy constraints~\cite{maliah2016collaborative}. 


%Beyond developing all these approaches for generating provisional plans, an important contribution of the proposed research is in performing a thorough experimental comparison of these approaches over a range of domains. 
We do not expect any of the proposed approaches to dominate the others and thus a thorough experimental comparison will be performed to give insights as to how to select a suitable approach for a given MAP problem. Algorithm selection techniques may be used for this purpose. In addition, we will develop methods for generating provisional plans that integrate the above approaches, such as performing coordinated planning for subsets of agents on a relaxed MAP problem. 



\subsection{Analyzing and Refining the Provisional Plans}
\label{sec:analyzing}
The next stage in IOP is to continuously refine the generated provisional plans by searching for PIFs and resolving them (lines~\ref{alg:identifyPIFs},\ref{alg:identifyPIFs2}, and~\ref{alg:resolve} in Alg.~\ref{alg:IOP}). 
...
In MAP models that are based on propositional logic like MA-STRIPS~\cite{brafman2013complexity}, we formally define PIF by the tuple $\langle A,T,D\rangle$, where $A$ is the group of involved agents, $T$ is the type of PIF -- positive (denoted by P) or negative (denoted by N), and 
$D$ depends on the PIF type: for positive PIFs $D$ is the fact that is needed by the provisional plans of the agents in $A$, and for negative PIFs $D$ is the pair of actions that conflict. For example, the positive PIFs in Figure~\ref{fig:free-space} are  
...
%If the provisional plans were generated by planning under the generalized free-space assumption, then searching for PIFs is, in a sense, checking the validity of this assumption. %e generalized free-space assumption adopted at the time that these plans were generated. 
Searching for PIFs in a set of provisional plans is, in a sense, checking the validity of the assumptions adopted when these plans were generated (e.g., the generalized free-space assumption). The methods we will develop for identifying PIFs and their complexity will vary, depending on PIF type(negative or positive) and the MAP formalism used. For instance, identifying PIFs in a temporal planning setting is trivial: every action in the provisional plans is associated with a specific time, and PIFs can be detected by simulating the execution of the provisional plans.  For MAP models that are not temporal, identifying PIFs is more challenging, requiring, methods for identifying causal-link threats and open preconditions~\cite{cox2009efficient} -- every open precondition and every unresolvable causal-link threat is a PIF. Moreover, additional PIFs may be detected if we have some knowledge about the timing and duration of the planned actions. %performing each action and the time when agents plan to perform them. 


\subsubsection{Methods for Resolving PIFs}
\label{sec:methodsForResolving}
We will study two approaches for resolving a given PIF $\langle A,T,D \rangle$:  ``join and replan'' and ``constrain and replan''. 
The ``join and replan'' method considers the agents in $A$ as a single agent and replans for them together a baseline MAP solver. By construction, the originating PIF is resolved, as the generated plan is free of any PIFs between these agents. The complexity of the ``join and replan'' method depends on the MAP planner used, and can be exponential in $|A|$. %the number of joined agents. 
The ``constrain and replan'' method attempts to avoid this exponential complexity by replanning for only one of the agents in $A$, constraining that agent's new plan to avoid the PIF. For a negative PIF, this constraint means avoiding the actions that caused it. For a positive PIF, it  means adding a subgoal of performing the task assumed by another agent's plan. 
Coordinating agents by incrementally adding constraints has been explored by Ephrati and Rosenschein~\cite{ephrati1993multi} but their focus was on self-interested agents aiming to avoid sharing their private goals and preferences. %that are self-interested, and adding constraints incrementally was done to avoid sharing private goals and preferences of the individual agents. 
...
\caption{\small{Rectangles represent IRT nodes: the upper part of the rectangle shows the provisional plans, the lower part shows the PIFs. Constraints are not shown for simplicity. The IRT root node consists of the provisional plans generated using the generalized free-space assumption (see Figure~\ref{fig:free-space}). Every branch corresponds to choosing a PIF and resolving it. The chosen PIF and resolution method is given in the rounded rectangles.}}\textsl{} %Any tree search algorithm can traverse this tree, and more branches can be generated. The goal node is an IRT node representing a joint plan without any PIF, and our example IRT has two possible paths to it.}}
...
it ignores positive PIFs, assumes a given allocation of goals to agents, and do not consider deferring PIF resolution to execution (see Section~\ref{sec:resolving}). 
...
Using a systematic heuristic search algorithm to search the IRT provides {\bf flexible control over the cost of the resulting joint plan}.\footnote{The cost of a joint plan can be defined in multiple ways, and we will focus on joint plan cost functions that aggregate the costs of the joint plan's constituent individual plans (e.g., summing their costs).} 
...
%For practical purposes optimality must often be relaxed. 
%Finding an optimal joint plan is often not needed, and quickly obtaining a suboptimal joint plan is preferred. Moreover, finding optimal joint plans can be intractable in practice. Thus, we propose to develop 

We propose to develop and analyze several approaches for trading optimality for runtime in IOP, finding suboptimal solutions in cases where finding optimal solutions is not feasible. % introducing suboptimality and study how to combine these approaches can be combined to achieve the best performance. 

% We could use bounded suboptimal search algorithms but it requires some modifications
{\bf Suboptimal search of the IRT.} 
This approach introduces suboptimality by searching the IRT with a suboptimal search algorithm. 
Moreover, it is possible to control the amount suboptimality introduced by using {\em bounded-suboptimal search algorithms}. Most bounded-suboptimal search algorithms~\cite{pohl1973avoidance,pearl1982studies,thayer2011bounded} rely on an evaluation function in the form of $f_w(n)=g(n)+w\cdot h(n)$, where $w$ is the desired amount of suboptimality, $h(n)$ is a lower bound on the cost of a path from $n$ to a goal, and $g(n)$ is the cost spent to reach $n$ from the start state. Adapting such algorithms to search the IRT for suboptimal solutions is non-trivial, as there is no obvious lower bounding heuristic function ($h(n)$), since there is no clear notion of ``path cost'' in the IRT, as cost is associated with nodes (the cost of the incumbent joint plan) and not edges. 

%since the notion of ``path cost'' between nodes is meaningless in the IRT, as cost is associated with nodes (the cost of the incumbent joint plan) and not edges. 

% This can be solved with EES, but we need distance and inadmissible estimates 
To meet this challenge, we will explore an approach based on Explicit Estimation Search (EES)~\cite{thayer2011bounded}, a state-of-the-art bounded suboptimal search algorithm. To choose which nodes to expand, EES uses a combination of evaluation functions: $f(\cdot)$, $\hat{f}(\cdot)$, and $\hat{d}(\cdot)$. $f(n)$ and $\hat{f}(n)$ are both estimates of the cost of the optimal path to a goal that passes through $n$. $f(n)$ is an admissible (lower bound) estimate of the optimal cost, while $\hat{f}(n)$ is inadmissible, and thus potentially more accurate than the $f(n)$. $\hat{d}(\cdot)$ is an estimate of the number of operators needed to reach the goal, respectively. The $\hat{d}(\cdot)$  heuristic serves as rough estimates of the search effort required to find  a goal beneath node $n$ with the desired suboptimality. EES intelligently combines $f(n)$, $\hat{f}(n)$, and $\hat{d}(\cdot)$ to effectively find a solution whose cost is at most $w$ times the optimal solution, where $w$ is a parameter chosen by the user.


Applying EES to find joint plans in the IOP tree requires developing effective $f(n)$, $\hat{f}(n)$, and $\hat{d}(\cdot)$ suitable for searching the IOP tree. If $cost(n)$ is an admissible lower bound, it can be used instead of $f(n)$. If not, we will investigate other ways to generate lower bounds on $cost^*(n)$, e.g., by using cost-partitioning techniques on the provisional plans such as those used for adding multiple abstraction heuristics~\cite{katz2010optimal}. We have begun to explore ways for analyzing the PIFs in the provisional plans to estimate the distance to a goal ($\hat{d}(\cdot)$), with preliminary results suggesting that this is a promising approach~\cite{barer2014suboptimal-socs}. We will also explore using machine learning methods to learn from past planning sessions the relation between various problem properties and the most suitable PIF resolution mechanism, thus providing additional guidance for searching the IRT. 
...
The above approaches for adding suboptimality rely on having a lower bound on $cost^*(n)$. Obtaining such a lower bound may be difficult in some domains. An alternative approach we will explore is to {\bf guide the BFS of the IRT with an inadmissible estimate of $cost^*(n)$}, i.e., estimates that may not be a lower bound. Such heuristic can be obtained, for example, by applying Machine Learning methods to learn an estimator $\widehat{cost}(n)$ of $cost^*(n)$ from MAP problems solved in the past. 
...
A key question is which PIFs can be ignored when planning. 
This question can be framed as a special case of the SharedPlans ``can bring about group'' predicate (\cbag), which states that a group of agents is able to achieve some task~\cite{grosz1996collaborative}. In the context of resolving PIFs, the task referred to by the \cbag\ predicate is the agents achieving their goals, given the incumbent joint plan and its PIFs.  
...
Computing \cbag\ efficiently for a set of provisional plans presents both theoretical and algorithmic challenges, requiring the prediction of what the agents can achieve without generating a plan to achieve it. We will investigate the following approaches to do so. %computing $cbag$. 
...
For example, robots may have predefined {\em behaviors} (corresponding to recipes in SharedPlans terminology) for detecting and avoiding collision with other agent and for swapping locations with an adjacent robot. Such behaviors are sufficient to enable all robots to reach their goal locations under very general settings~\cite{wang2009tractable,snape2011hybrid,de2013push}. 
Consequently, even if we do not plan to avoid collisions during the planning phase, we are still guaranteed that the agent will not collide. 
This allows much faster planning as PIFs related to collisions can be ignored, and computational resources can be allocated to resolve other PIFs. For example, searching the IRT in Figure~\ref{fig:irt-example} could halt earlier if the negative PIF: $\langle [V,S], N, \{ \at{V}{4}{(2,1)}, \move{S}{4}{(2,1)} \} \rangle$ is ignored.

%Importantly, there is no need during planning time to plan exactly how every specific potential collision will be handled, as we know that every collision will be detected and avoided. This allows much faster planning as PIFs related to collisions can be ignored during planning, and computational resources can be allocated to resolve other PIFs. For example, searching the IRT in Figure~\ref{fig:irt-example} could halt earlier if the negative PIF: $\langle [V,S], N, \{ \at{V}{4}{(2,1)}, \move{S}{4}{(2,1)} \} \rangle$ is ignored.
...
Estimating whether the agents will be able to resolve a PIF can be done by trying to resolve it in a relaxed version of the original MAP problem.  The high-level idea we propose is to estimate \cbag\ for a PIF in an incumbent  joint plan by checking if that PIF can be resolved in the relaxed MAP problem. Possible ways to relax a given MAP problem which we will investigate include ignoring delete effects of agents' actions~\cite{hoffmann2005ignoring,domshlak2015red,vstolba2014relaxation} and abstracting some of the state variables~\cite{edelkamp2001planning,korf2002disjoint,felner2004additive,helmert2014merge,maliah2016collaborative}. An example of the latter type of relaxation is to ignore limitations on robots' motion that are due to kinematic constraints.
...
We successfully applied this approach in preliminary work~\cite{maliah2016collaborative}. 
%We successfully applied this approach in our preliminary work on GPPP for privacy-preserving MAP~\cite{maliah2016collaborative}. 
...
Exactly computing $\Delta_x(n)$ is not possible, since $cost^*(n)$ is not known before the MAP problem is solved and $xcost(n)$ is not likely to be known prior to execution. To bound the suboptimality of a returned joint plan, we will explore methods for bounding $\Delta_x(n)$. 
An approach we intend to take is to generate a lower bound heuristics, $h(n)$$\leq$$cost(n)$, and an upper bound heuristic $h_x(n)$$\geq$$xcost(n)$, and then bounding $\Delta_x(n)$ by $h_x(n)$$-$$h(n)$. We will investigate generating $h_x(n)$ using approaches similar to those used for estimating \cbag. %; namely, $xcost(n)$ can be estimated by analyzing existing recipes for resolving PIFs (e.g., collision avoidance protocols), or by considering a relaxed version of the original MAP problem where estimating the execution cost can be done faster. 

Deferring handling PIFs to execution also introduces a probability of failure, as some PIFs cannot be resolved during execution. We will also study the relation between this probability, the expected plan cost, and overall planning time. %This introduces a natural tradeoff,probability of failure is another Thus, ignored PIF also adds  
...
To demonstrate the generality of IOP, we will evaluate it empirically on a range of MAP problems.


{\bf 1. MA-STRIPS~\cite{brafman2013complexity}.} This is the main setting we will use, as it can be viewed as the simplest extension of single agent planning to MAP. MA-STRIPS is a convenient domain for MAP research, as there is an international competition for comparing MA-STRIPS planners~\cite{vstolba2015competition} and the competition organizers graciously made the code for all competing planning algorithms as well as all the problems used in this competition available to the community. This allows comparing the IOP-based MA-STRIPS planning algorithms we will develop against the state-of-the-art MA-STRIPS planners on a standard set of benchmark domains. %MA-STRIPS serves as a ``lab'' setting, and we intend to evaluate IOP on a range of problems and settings listed below.

{\bf 2. Search-Based Planning for Teams of Robots.}
We will implement IOP on top of the Search-Based Planning Library (SBPL)~\cite{likhachev2010sbpl}, a successful framework for applying search-based planning algorithms for robots. SBPL 
was used in a range of robotic platforms~\cite{stern2014potential,aine2015learning,cohen2014planning,phillips2012graphs,likhachev2009planning} and it is integrated with the Robotic Operating System (ROS) to allow relatively easy deployment to real robots and integration with standard ROS simulators. For the proposed research we have started to develop a multi-robot extension of SBPL. See the attached letter of support from Prof. Maxim Likhachev, who developed SBPL.  

%(MR-SBPL) and a preliminary study in this domain where we compared planning with \astar\ to a simplified version of IOP showed promising results. 


%such as a single robotic arm~\cite{stern2014potential,aine2015learning}, multiple robotic arms~\cite{cohen2014planning}, Willow Garage’s PR2~\cite{phillips2012graphs}, and an autonomous vehicle~\cite{likhachev2009planning}. We have started to develop a multi-robot extension of SBPL (MR-SBPL) and a preliminary study in this domain where we compared planning with \astar\ to a simplified version of IOP showed promising results. 
%Implementing IOP on real robots is beyond the scope of this research and requires additional expertise and funds. But, as a step towards realizing IOP on robot teams we will implement IOP on top of the Search-Based Planning Library (SBPL)~\cite{likhachev2010sbpl}, a successful framework for applying search-based planning algorithms for robots. SBPL is integrated with the Robotic Operating System (ROS), to allow relatively easy deployment to real robots and integration with standard ROS simulators. It was used in a range of robotic platforms such as a single robotic arm~\cite{stern2014potential,aine2015learning}, multiple robotic arms~\cite{cohen2014planning}, Willow Garage’s PR2~\cite{phillips2012graphs}, and an autonomous vehicle~\cite{likhachev2009planning}. We have started to develop a multi-robot extension of SBPL (MR-SBPL) and a preliminary study in this domain where we compared planning with \astar\ to a simplified version of IOP showed promising results. 


{\bf 3. Planning for Data Mules.} Data mules is a rising architecture for maintaining low-cost high-latency networks, where mobile agents carry data between the network nodes~\cite{shah2003data}. %E.g., when  deploying a communication network is not feasible.
There are several planning tasks for data mules, including route planning~\cite{sugihara2011path} and handling network node failures~\cite{crowcroft2016using}. We will evaluate IOP against existing solutions for data mule planning, providing both practical benefit to as well as encourage cross fertilization with other communities.



{\bf 4. Dec-POMDP~\cite{bernstein2002complexity}.} This very general MAP formalism considers both partial observability and stochastic action outcomes. While it has been applied in practice in some problems, scaling Dec-POMDP solvers is extremely difficult, and thus pose an opportunity to demonstrate the usefulness of IOP. We aim to collaborate on this with Prof. Christopher Amato, an expert in Dec-POMDPs (see requested funding to support this collaboration) and build on top of the MADP toolbox of Dec-POMDP algorithms~\cite{MADPToolbox-0.3.1}. % and collaborating with Prof. Christopher Amato, an expert in Dec-POMDPs (see requested funding to support this collaboration). %To support this development we intend to collaborate with Prof. Amato (additional funding is request ). 
% of Dec-POMDP algorithms~\cite{MADPToolbox-0.3.1}. 

Lastly, we will experiment with applying IOP to solve single agent STRIPS problems, using 
existing domain decomposition methods~\cite{nissim2012tunneling,amir2003factored} and applying IOP on the resulting sub-domains. 
%over single agent problems  . To this end we will experiment on standard domain-independent planning problems, and explore  using IOP with various domain decomposition methods~\cite{nissim2012tunneling,amir2003factored}.
%{\bf 5. Single Agent Search.} As noted above, we also intend to experiment with applying IOP to solve single agent problems. To this end we will experiment on standard domain-independent planning problems, and explore  using IOP with various domain decomposition methods~\cite{nissim2012tunneling,amir2003factored}. 
